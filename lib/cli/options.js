...
/**
 * Given path to `package.json` in `args.package`, attempt to load config from `htmlsanitycheck` prop.
 * @param {Object} [args] - Arguments object
...
const loadPkgRc = (args = {}) => {
  ...
  result = {};
  const filepath = args.package || findUp.sync(htmlsanitycheckrc.package);
  if (filepath) {
    try {
 ...
 * 3. `htmlsanitycheck` prop of `package.json`
...
const loadOptions = (argv = []) => {
  ...
  args = parse(args._, htmlsanitycheckrc, args, rcConfig || {}, pkgConfig || {})
