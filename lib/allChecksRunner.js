    ...
    fs.readdir(this.config.sourceDir, { recursive: true, withFileTypes: true }, (err, files) => {
      if (!err) {
        files.filter((file) => {
          ...
          //const ImageMapChecker = require('./checker/ImageMapChecker')
          //singlePageResult.addSingleCheckResult(new ImageMapChecker(this.config).performCheck(htmlPage))
          const BrokenHttpLinksChecker = require('./checker/BrokenHttpLinksChecker')
          singlePageResult.addSingleCheckResult(new BrokenHttpLinksChecker(this.config).performCheck(htmlPage))
