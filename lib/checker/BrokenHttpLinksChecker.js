...
// URL cache to avoid redundant HTTP requests - static to persist across instances
const httpUrlCache = new Map()
...
class Logic extends IBrokenHttpLinksChecker.DecisionTableLogic {
  ...
  check (href) {
    // Check if we have a complete cached result for this URL with this config
    if (!httpUrlCache.has(href)) {
      // Execute the decision table logic normally
      const model = {
        href,
        findings: [],
        config: this.config,
      ...
      httpUrlCache.set(href, { countChecks: 1, findings: model.findings.slice() })
    }
    const cachedResult = httpUrlCache.get(href)
    // Return a shallow copy of the cached result, with a shallow copy of findings
    return {
      countChecks: cachedResult.countChecks,
      findings: cachedResult.findings.slice()
    }
  }
  ...
  doExecuteHttpRequest (/* ExecuteHttpRequest */ arg, model) {
    // Execute actual HTTP request as normal
    ...
    try {
      ...
      model.responseStatusCode = 500
      model.locationHeader = undefined
    }
  }
