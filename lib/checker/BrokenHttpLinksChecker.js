...
class Logic extends IBrokenHttpLinksChecker.DecisionTableLogic {
  ...
  isCheckHttpStatusCodeInConfiguredSuccessRange_SUCCESS (model) {
    return model.responseStatusCode >= 200 && model.httpResponse.statusCode < 300
  }
  ...
  doExecuteHttpRequest (/* ExecuteHttpRequest */ arg, model) {
    ...
    http.request(model.href, { method: arg.getSymbol(), timeout: 5000 }, (res) => {
      model.responseHeaders = res.headers
      model.responseStatusCode = res.statusCode
    }).on('error', (error) => {
      if (error.code in ['ECONNREFUSED', 'ETIMEDOUT']) {
