...
class BrokenHttpLinksChecker extends Checker {
  ...
  createSonarRule () {
    const CleanCodeAttribute = require('./../reporter/sonar/CleanCodeAttribute')
    const Impact = require('./../reporter/sonar/Impact')
    const SoftwareQuality = require('./../reporter/sonar/SoftwareQuality')
    const Severity = require('./../reporter/sonar/Severity')
    const Rule = require('./../reporter/sonar/Rule')
    return new Rule(this.constructor.name.toLowerCase(), this.constructor.name, 'External Links Check', CleanCodeAttribute.TESTED, [new Impact(SoftwareQuality.SECURITY, Severity.HIGH)])
  }
